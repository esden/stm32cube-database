...
[/#list]
{
  "Application": {
    ...
    "ApplicationTemplate": {
      ...
      "PostGenerateCommand": [#if underRoot?? && underRoot=="true"]"touchgfx update_project --project-file=[#if dualCore??]../[/#if]../.cproject "[#else]"touchgfx update_project --project-file=${ideProjectFile?replace(MXPRojectRootPath,"..")}" [/#if],      
      [#else]
