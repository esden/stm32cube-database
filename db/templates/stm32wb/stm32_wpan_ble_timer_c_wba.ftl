...
{
   /* If there is no more space and the timer does not exist already, return error */
  if(BLE_TIMER_GetFromList(&BLE_TIMER_List, id) == NULL && LST_get_size(&BLE_TIMER_List) == TIMER_MAX_ENTRIES)
  {
  ...
  BLE_TIMER_t *timer = UTIL_MM_GetBuffer(sizeof(*timer));
  if(timer == NULL){
    return BLE_STATUS_INSUFFICIENT_RESOURCES;
  }
